...
try:
    while True:
        ...
        if (t2 - t1) > 1000:
            print("Reading of IMU gyro data took more than 1 second!")
        gyro_x = (gyro_data[0] << 8) | gyro_data[1]
        gyro_y = (gyro_data[2] << 8) | gyro_data[3]
        gyro_z = (gyro_data[4] << 8) | gyro_data[5]
        if gyro_x >= 32768: gyro_x = ((65535 - gyro_x) + 1) * -1 # convert unsigned ints to signed ints (so there can be negatives)
        ...
        t2:int = time.ticks_ms()
        if (t2 - t1) > 1000:
            print("Reading of IMU accel data took more than 1 second!")
        ...
        if excess_us > 0:
            if excess_us > 1000000:
                print("Excess wait time more than 1 second!")
